...
uint8_t hallToMotor[8] = {255, 255, 255, 1, 5, 1, 4, 255};  // Default hall table. Overwrite this with the output of the hall auto-identification
// uint8_t hallToMotor[8] = {255, 2, 0, 1, 4, 3, 5, 255};  // Example hall table

const int THROTTLE_LOW = 500;               // ADC value corresponding to minimum throttle, 0-4095
const int THROTTLE_HIGH = 4000;             // ADC value corresponding to maximum throttle, 0-4095

const bool CURRENT_CONTROL = true;          // Use current control or duty cycle control
const int PHASE_MAX_CURRENT_MA = 2500;      // If using current control, the maximum phase current allowed
const int BATTERY_MAX_CURRENT_MA = 1500;    // If using current control, the maximum battery current allowed
...
const uint A_PWM_SLICE = 1;
const uint B_PWM_SLICE = 2;
const uint C_PWM_SLICE = 3;
...
const int CURRENT_SCALING = 3.3 / 0.0007 / 40 / 4096 * 1000;
...
void writePhases(uint ah, uint bh, uint ch, uint al, uint bl, uint cl)
{
    ...
    pwm_set_both_levels(A_PWM_SLICE, 255 - al, ah);
    pwm_set_both_levels(B_PWM_SLICE, 255 - bl, bh);
    pwm_set_both_levels(C_PWM_SLICE, 255 - cl, ch);
}
...
void init_hardware() {
    ...
    pwm_config_set_output_polarity(&config, true, false);   // Invert the lowside PWM such that 0 corresponds to lowside transistors on
    ...
    pwm_set_mask_enabled(0xE); // Enable our three PWM timers
}
